...
class DeepSpeedEngine(Module):
    ...
    def _do_optimizer_sanity_check(self, basic_optimizer):
        ...
        # data type checks
            ...
            raise NotImplementedError(
                "unsupported mix of model dtype and gradient accumulation type"
            )
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        if client_optimizer is not None:
            ...
            if self.zero_use_cpu_optimizer() and not isinstance(
                basic_optimizer, deepspeed.ops.adam.DeepSpeedCPUAdam
            ):
        ...
        log_dist(
            "DeepSpeed Basic Optimizer = {}".format(basic_optimizer.__class__.__name__),
            ranks=[0],
        )
    ...
    def _configure_fp16_optimizer(self, optimizer):
        ...
        if isinstance(optimizer, fused_opts) or self.optimizer_name() in [
            ...
            log_dist(
                f"Creating fp16 unfused optimizer with dynamic loss scale", ranks=[0]
            )
    ...
    def allreduce_gradients(self, bucket_size=MEMORY_OPT_ALLREDUCE_SIZE):
        assert not (
            self.bfloat16_enabled() and self.pipeline_parallelism
