...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-13b-neox-eus-v1.1-ema
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_13B_eus_v1.1_ema.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    save_load/${CONF_NAME}_13B_eus_v1.1_ema.yml
