...
#SBATCH --output=.slurm/latxa-7b-v1.1_l2loss.out
#SBATCH --error=.slurm/latxa-7b-v1.1_l2loss.err
#SBATCH --exclusive
#SBATCH --requeue
...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-eus-l2loss_29072024
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_7B_eus_v1.1_500k_l2loss.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    save_load/${CONF_NAME}_7B_eus_v1.1_500k_l2loss.yml
