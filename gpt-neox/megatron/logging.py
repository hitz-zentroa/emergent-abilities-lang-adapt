...
def training_log(
    ...
    logged_perparam_gradient = False
    if (
        ...
        if neox_args.log_grad_pct_zeros or neox_args.log_grad_norm:
            #print(f"\n\nStoring Gradients for type: { type(model)} \n\n")
            model.store_gradients = True  # start storing gradients
            #print(f"Made it: {model.stored_gradients}")
        for i, (name, param) in enumerate(model.module.named_parameters()):
            ...
            if neox_args.log_grad_norm:
                ...
                #print(f"\nModel Has Stored Gradients: { type(model) }\n") #[param for param in dir(model)  if '__' not in param ]}
                #print(f"\nHas Attribute: {model.stored_gradients}")
                if (
                    hasattr(model, "stored_gradients")
                    and model.stored_gradients is not None
                ):  
                    # 
                    print("\nAble to log gradient\n")
                    grad = model.stored_gradients[i]
                    if grad is not None:
                        tb_wandb_log(
    ...
    if not logged_perparam_gradient:
