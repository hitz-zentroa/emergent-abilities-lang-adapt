...
def pretrain(neox_args):
    ...
    for i, (name, param) in enumerate(model.module.named_parameters()):
        initial_checkpoint_params[name] = copy.deepcopy(param)
    print(f"LENGTH = {len(initial_checkpoint_params)}")  
...
def update_moving_average(ma_model, current_model):
    ...
    for i, (name, param) in enumerate(current_model.module.named_parameters()):
        ...
        if name in ma_model:
            param.data.sub_((1 - 0.92) * (ma_model[name].data - param.data)) #lerp_(ma_model[name].data, 1. - current_decay)
...
def train_step(neox_args, timers, data_iterator, model, optimizer, lr_scheduler, initial_checkpoint_params=None):
...
def train(
    ...
    train_data_iterator,
    valid_data_iterator,
    initial_checkpoint_params=None
):
