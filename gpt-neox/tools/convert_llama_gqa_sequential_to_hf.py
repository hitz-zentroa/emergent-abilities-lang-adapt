...
try:
    ...
    print(
        "LLamaForCausalLM could not be imported. Please update your `transformers` installation and try again."
    )
    raise Exception
...
def load_partitions(
    input_checkpoint_path, mp_partitions, use_cuda: bool = True
) -> List[torch.Tensor]:
    ...
    if not get_key(neox_config, "mlp-type") == "llama":
        ...
        hf_config = LlamaConfig(
            ...
            intermediate_size=compute_intermediate_size(
                get_key(neox_config, "hidden-size"),
                ffn_dim_multiplier=get_key(neox_config, "ffn_dim_multiplier", 1.0),
                multiple_of=get_key(neox_config, "multiple_of", 256),
            ),
            hidden_act=get_key(neox_config, "activation", default="silu"),
            max_position_embeddings=get_key(neox_config, "max-position-embeddings"),
            initializer_range=get_key(neox_config, "init-method-std", 0.02),
            rms_norm_eps=float(get_key(neox_config, "rms-norm-epsilon", 1.0e-6)),
            use_cache=True,
...
def convert(
    input_checkpoint_path, loaded_config, output_checkpoint_path, use_cuda: bool = True
):
    ...
    loaded_tp_ranks = load_partitions(
        input_checkpoint_path, mp_partitions, use_cuda=use_cuda
    )
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        for key, hf_key in {
            ...
            # if key in loaded_tp_ranks[0].keys():
            state_dict[hf_key] = (
                sum(get_state(loaded_tp_ranks, key, layer_i + 2))
            ) / len(loaded_tp_ranks)
        ...
        for key in ["attention.query_key_value.weight"]:
            ...
            print(
                sharded_qkv.shape
            )  # -> should have shape [TP_SIZE, (hidden_size + 2 * kv_hidden_size) / TP_SIZE, hidden_size]
    ...
    for key, hf_key in {
        ...
        key = "norm." + key
        # if key in loaded_tp_ranks[0].keys():
        norm_state_dict[hf_key] = (
            sum(
                get_state(
                    loaded_tp_ranks, key, get_key(loaded_config, "num-layers") + 3
                )
            )
        ) / len(loaded_tp_ranks)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_cuda", action="store_true", help="Set to true in order to ignore CUDA."
    )
    ...
    hf_model = convert(
        args.input_dir, loaded_config, args.output_dir, use_cuda=not args.no_cuda
    )
