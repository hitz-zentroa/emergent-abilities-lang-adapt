...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-eus-curriculum_first_500_steps
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
    ...
    LOAD_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-TP-${TP}
    FINETUNE="true"
    CONF_NAME="start"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_7B_eus_v1.1_curriculum_first_500_steps.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    base_config_chkpt_freq10.yml \
    models/llama-2-7b.yml \
    deepspeed/zero1.yml \
    hyperparameters/7B_v1.1_curriculum_first_500_steps.yml\
    data/latxa-v1.1.yml \
    save_load/${CONF_NAME}_7B_eus_v1.1_curriculum_first_500_steps.yml
