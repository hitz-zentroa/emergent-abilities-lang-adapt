...
#SBATCH --output=.slurm/latxa-7b-v1.1_ema_decay0.992_interval1.out
#SBATCH --error=.slurm/latxa-7b-v1.1_ema_decay0.992_interval1.err
#SBATCH --exclusive
#SBATCH --requeue
...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-eus-decay0.992-interval1
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_7B_eus_v1.1_500k_ema_decay0.992_interval1.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    save_load/${CONF_NAME}_7B_eus_v1.1_500k_ema_decay0.992_interval1.yml
