...
#SBATCH --output=.slurm/latxa-7b-v1.1_eng_first500stepx.out
#SBATCH --error=.slurm/latxa-7b-v1.1_eng_first500stepx.err
#SBATCH --exclusive
#SBATCH --requeue
...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-eus-latxa-7b-v1.1_eng_first500stepx
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_latxa-7b-v1.1_eng_first500stepx.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    data/latxa-v1.1.yml \
    save_load/${CONF_NAME}_latxa-7b-v1.1_eng_first500stepx.yml
