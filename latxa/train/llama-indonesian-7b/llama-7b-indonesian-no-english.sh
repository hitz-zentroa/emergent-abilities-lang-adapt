...
#SBATCH --output=.slurm/llama_2_7b_indonesian_noenglish.out
#SBATCH --error=.slurm/llama_2_7b_indonesian_noenglish.err
#SBATCH --exclusive
#SBATCH --requeue
...
SAVE_PATH=/leonardo_scratch/large/userexternal/amohamed/Llama-2-7b-neox-indonesian-noenglish
if [ -d "$SAVE_PATH" ]; then
    LOAD_PATH=$SAVE_PATH
    FINETUNE="false"
    CONF_NAME="continue"
...
printf '{\n "save": "%s",\n "load": "%s",\n "finetune": %s\n}' ${SAVE_PATH} ${LOAD_PATH} $FINETUNE \
    >${HOME}/latxa/configs/save_load/${CONF_NAME}_Llama2_7b_neox_indonesian_noenglish.yml
...
python $TRAIN_PATH/deepy.py $TRAIN_PATH/train.py \
    ...
    save_load/${CONF_NAME}_Llama2_7b_neox_indonesian_noenglish.yml
